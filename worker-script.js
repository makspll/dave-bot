...
const KEYWORDS = {
    "ai" : {
      ...
      "gpt_prompt": "Tell the previous user that A.I. sucks in a very condescending way while being very aloof in extremely simple language"
    },
    "gpt" : {
      "chance": 0.8,
      ...
      "gpt_prompt": "Tell the previous user that chat GPT sucks in a very condescending way while being very aloof in extremely simple language"
    },
    "gadwick" : {
      "chance": 1,
    ...
    "job": {
      ...
      "gpt_prompt": "Tell the previous user that they seriously need to get a job, or if they said they already have one that's bad, that they need to get a new job, in a very condescending way while being very alloof and in extremely simple language"
    },
    "dave": {
      "chance": 0.5,
      ...
      "gpt_prompt": "Tell the previous user what you think about them, let loose."
    }
}
    ...
    if (request.method === "POST") {
      ...
      if ('message' in payload && payload.message.text) { 
        ...
        if (!anti_spam_mode){ 
          ...
          anyTriggered = anyTriggered | await keywords(words,  payload.message.chat.id, env.TELEGRAM_API_KEY, env.OPEN_AI_KEY);
...
async function call_gpt(system_prompt, message_history, openAI_key) {
    let messages = [{
        "role": "system",
...
async function keywords(words, chatId, apiKey, openAI_key) {
  ...
  if (triggers.length > 0) {
    ...
    if (Math.random() < trigger.chance) {
        if(trigger.gpt_prompt && (Math.random() < KEYWORD_GPT_CHANCE)) {
            ...
            let response = await call_gpt(SYSTEM_PROMPT + "." + trigger.gpt_prompt, [words.join(",")], openAI_key);
            if (response) {
